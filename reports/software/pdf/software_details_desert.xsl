        ...
        <fo:flow flow-name="xsl-region-body">
          ...
          <fo:block font-size="{$fontSize}">
            ...
            <fo:table table-layout="fixed" width="100%" border-collapse="separate" background-color="{$textColor}" space-after="10" space-before="10">
              ...
              <fo:table-column column-width="1{$pictureSize}" />

              <fo:table-body>
                <fo:table-row height="{$rowHeight}">
                  <fo:table-cell background-color="{$labelColor}" padding-top="{$paddingTop}" padding-left="{$paddingLeft}" padding-bottom="10">
                    <fo:block font-weight="bold">Description</fo:block>
                  </fo:table-cell>
                        
                  <fo:table-cell padding-top="{$paddingTop}" padding-left="{$paddingLeft}" padding-bottom="10">
                ...
                <fo:table-row height="{$rowHeight}">
                  <fo:table-cell background-color="{$labelColor}" padding-top="{$paddingTop}" padding-left="{$paddingLeft}">
                    <fo:block font-weight="bold">Developed by</fo:block>
                  </fo:table-cell>

                  <fo:table-cell padding-top="{$paddingTop}" padding-left="{$paddingLeft}">
                    <fo:block><xsl:for-each select="developers/developer"><xsl:value-of select="name"/>&#160;</xsl:for-each></fo:block>
                  </fo:table-cell>
                </fo:table-row>

                <fo:table-row height="{$rowHeight}">
                  <fo:table-cell background-color="{$labelColor}" padding-top="{$paddingTop}" padding-left="{$paddingLeft}">
                    <fo:block font-weight="bold">Published by</fo:block>
                  </fo:table-cell>

                  <fo:table-cell padding-top="{$paddingTop}" padding-left="{$paddingLeft}">
                    <fo:block><xsl:for-each select="publishers/publisher"><xsl:value-of select="name"/>&#160;</xsl:for-each></fo:block>
                  </fo:table-cell>
                </fo:table-row>

                <fo:table-row height="{$rowHeight}">
                  <fo:table-cell background-color="{$labelColor}" padding-top="{$paddingTop}" padding-left="{$paddingLeft}">
                    <fo:block font-weight="bold">Platform</fo:block>
                  </fo:table-cell>

                  <fo:table-cell padding-top="{$paddingTop}" padding-left="{$paddingLeft}">
                    <fo:block><xsl:value-of select="platform"/></fo:block>
                  </fo:table-cell>
                </fo:table-row>
          ...
          <xsl:if test="picture-front != '' or screenshot-one != '' or screenshot-two != ''">
            <fo:table table-layout="fixed" width="100%" border-collapse="separate">
              <xsl:if test="picture-front != ''"><fo:table-column column-width="33%" /></xsl:if>
              <xsl:if test="screenshot-one != ''"><fo:table-column column-width="33%" /></xsl:if>
              <xsl:if test="screenshot-two != ''"><fo:table-column column-width="33%" /></xsl:if>
              ...
              <xsl:if test="screenshot-one != ''">
                <fo:table-cell>
                  <fo:block>
                    <fo:external-graphic content-width="{$pictureSize}" content-height="{$pictureSize}" scaling="uniform">
                        <xsl:attribute name="src">url('<xsl:value-of select="screenshot-one" />')</xsl:attribute>
                    </fo:external-graphic>
                  </fo:block>
                </fo:table-cell>
              </xsl:if>
              ...
              <xsl:if test="screenshot-two != ''">
                <fo:table-cell>
                  <fo:block>
                    <fo:external-graphic content-width="{$pictureSize}" content-height="{$pictureSize}" scaling="uniform">
                        <xsl:attribute name="src">url('<xsl:value-of select="screenshot-two" />')</xsl:attribute>
                    </fo:external-graphic> 
                  </fo:block>
                </fo:table-cell>
              </xsl:if>

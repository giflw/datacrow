...
<xsl:stylesheet version="1.0"
											...
											<xsl:if
												test="
										        	(
		        										...
		        										and audio-languages/language/name/text() != normalize-space($i18n-audio-language-default)
		        									)
												...
												<fo:block font-size="{$font-size-colum-title-lang}">
													...
													<xsl:choose>
														<xsl:when test="count(audio-languages/language/name) = 1">
															<xsl:if
																test="audio-languages/language/name/text() != normalize-space($i18n-audio-language-default)">
												...
												<fo:block>
													<xsl:call-template name="format-ratings">
														<xsl:with-param name="rating"
															select="number(substring(rating, 0, 2))" />
													</xsl:call-template>
												</fo:block>
											</fo:block>
										</fo:table-cell>
	...
	<xsl:template name="format-ratings">
		...
		<!--
			...
			<xsl:with-param name="rating" select="$rating"/> </xsl:call-template>
		-->

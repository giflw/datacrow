  ...
  <body>
    ...
    <xsl:for-each select="data-crow-objects/movie">
        ...
        <table width="600" style="border:0;" cellspacing="0" cellpadding="10">
            ...
            <tr><th>Actors</th>
                <td><xsl:for-each select="actors/actor">
                                   <xsl:value-of select="name"/>
                                 </xsl:for-each></td></tr>
            <tr><th>Directors</th>
                <td><xsl:for-each select="directors/director">
                                   <xsl:value-of select="name"/>
                                 </xsl:for-each></td></tr>
            <tr><th>Playlength</th>
          ...
          <tr>
            ...
            <td>
              <xsl:if test="picture-cd != ''">
                <img alt=""><xsl:attribute name="src"><xsl:value-of select="picture-cd" /></xsl:attribute></img>
              </xsl:if>
            </td>

            <td>
              <xsl:if test="picture-back != ''">
                <img alt=""><xsl:attribute name="src"><xsl:value-of select="picture-back" /></xsl:attribute></img>
              </xsl:if>
            </td>
          </tr>
        </table>
